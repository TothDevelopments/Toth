...
public class BottomNavigatorFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (getArguments().getInt("index", 0) == 0) {
			...
			View view = inflater.inflate(R.layout.fragment_lista_atividades, container, false);
			initFragmentListaConversas(view);
			return view;
		}else {
	...
	private void initFragmentListaConversas(View view) {
		...
		ArrayList<String> itemsData = new ArrayList<>();
		for (int i = 0; i < 4; i++) {
			itemsData.add("Grupo " + i);
		}

		BottomNavigatorConversasAdapter adapter = new BottomNavigatorConversasAdapter(itemsData);
		recyclerView.setAdapter(adapter);
	}
